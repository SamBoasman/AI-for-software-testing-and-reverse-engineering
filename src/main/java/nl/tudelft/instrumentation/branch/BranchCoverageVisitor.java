 ...
 * for parsing Java source code files and to visit the corresponding AST.
 *
...
public class BranchCoverageVisitor extends ModifierVisitor<Object> {
  ...
  /**
   * For if statement, we add coverage tracking in both true ({@code thenStmt}) and false
   * {@code elseStmt} branches
   */
  @Override
  public Node visit(IfStmt node, Object arg) {
    ...
    if (node.hasElseBranch()) {
      ...
      if (elseStmt instanceof BlockStmt) {
        ...
        block.addStatement(0,
            makeCoverageTrackingCall(filename, node.getBegin().get().line, false));
      } else {
        // creating block when it doesn't exist
        BlockStmt block = new BlockStmt();
  ...
  /**
   * For loops (for and while) We need to keep track of coverage for the entry block and the exit
   * case
   */
  ...
  protected MethodCallExpr createMethodCall(ConditionalExpr node, boolean branch,
    ...
    MethodDeclaration method = new MethodDeclaration(modifier, this.inferType(node), methodName);
    BlockStmt block = new BlockStmt();
  ...
  protected Type inferType(ConditionalExpr expr) {
    ...
    if (parent instanceof VariableDeclarator) {
      ...
      ClassOrInterfaceDeclaration clazz =
          expr.findAncestor(ClassOrInterfaceDeclaration.class).get();
