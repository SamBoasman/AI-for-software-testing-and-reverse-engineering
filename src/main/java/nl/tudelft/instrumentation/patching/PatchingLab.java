...
public class PatchingLab {
  ...
  static void initialize() {
    ...
    // This is completely up to you. You can pick a population of size X and you can create X number
    // of
    // random individuals. This would be your initial population. An individual in this could is a
    // list of
    ...
    // 5 binary operators used in the problem file, then an individual would look something like
    // this:
  ...
  public static void calcTarantula() {
    ...
    for (int operation_nr = 0; operation_nr < OperatorTracker.operators.length; operation_nr++) {
      ...
      float Tarantula = 0;
      // if operation_nr is on the hashmaps, then add get the value and use it to calculate
      // tarantula score.
      if (OP_Success.containsKey(operation_nr))
        Passed = OP_Success.get(operation_nr);
      if (OP_Failures.containsKey(operation_nr))
        Failed = OP_Failures.get(operation_nr);
  ...
  public static void getProbabilities() {
    ...
    double sum_fit =
        all_populations.stream().map(item -> item.fitness_score).reduce(0.0, (a, b) -> a + b);

    for (Population_individual p : all_populations) {

      double individual_score = p.fitness_score / sum_fit;
      System.out.println(
          "individual_score: " + p.fitness_score + " / " + sum_fit + " = " + individual_score);
  ...
  /**
   * One-point crossover: Lists all possible cut points and randomly selects one cut point from the
   * list
   */
  ...
  /**
   * 1) Randomly select one statement (node) to mutate among the top k suspicious statement 2)
   * Mutate the selected statement • Delete statement • Add statement (if, method call, etc.) •
   * Replace variable • Replace constant • Replace method call
   *
   * @param tarantula
   * @param index
   */
   ...
   *
   * Calculates the occurrence probabilities for each operator. Then create GENE_MUTATION_SIZE
   * amount of changes using the probability.
   *
  ...
  static void run() {
    ...
    // Tests are loaded from resources/tests.txt, make sure you put in the right tests for the right
    // problem!
  ...
  static String convertToCSV(String[] data) {
    return Stream.of(data).map(PatchingLab::escapeSpecialCharacters)
  ...
  static void writeToCSV() throws IOException {
    ...
    try (PrintWriter pw = new PrintWriter(csvOutputFile)) {
      data.stream().map(PatchingLab::convertToCSV).forEach(pw::println);
    }
  }
  ...
  private static class Population_individual {
    ...
    public Population_individual(double fitness_score, HashMap<Integer, Float> tarantula_score,
        String[] individual) {
    ...
    public String toString() {
      return "Population_individual: fitness_score=" + fitness_score + ", tarantula_score="
          + tarantula_score.toString();
    }
  }
}
