...
public class SymbolicExecutionLab {
  ...
  // Bookkeeping
  static HashSet<String> totalVisitedBranches = new HashSet<>(); // visitedBracnhes of best
                                                                 // permutation of all cycles
  ...
  static MyVar createVar(String name, Expr value, Sort s) {
    ...
    /**
     * Create var, assign value and add to path constraint. We show how to do it for creating new
     * symbols, please add similar steps to the functions below in order to obtain a path
     * constraint.
     */
   ...
   *
   * When you call solve, z3 will try to solve for the inputs considering your current path
   * constraint. If successful, it will tell you the inputs needed to satisfy the constraint. You
   * should use this to augment your fuzzer by trying to solve for the opposite side of the branch
  ...
  static void encounteredNewBranch(MyVar condition, boolean value, int line_nr) {
    ...
    if (value) {
      ...
      if (!unsatisfiables.contains(b_false)) {
        ...
        PathTracker.z3branches =
            c.mkAnd(c.mkEq(condition.z3var, c.mkBool(value)), PathTracker.z3branches);
      } else {
  ...
  /**
   * If satisfiable store trace for later processing. Use priority_queue to store and load traces.
   *
   * @param new_inputs
   */
  ...
  /**
   * Extends the satisfiable input trace to length of trace_length. This will ensure that
   * satisfiable input are explored a bit deeper. Satisfiable input traces with length > 25 will not
   * be extended anymore
   * 
   * @param trace
   * @return
  ...
  /**
   * If branch was found unsatisfiable by the solver, add to unsatisfiables HashSet to prevent from
   * solving again
   * 
   * @param new_branch
   */
  ...
  static List<String> fuzz(String[] inputSymbols) {
    /*
     * Add here your code for fuzzing a new sequence for the RERS problem. You can guide your fuzzer
     * to fuzz "smart" input sequences to cover more branches using symbolic execution. Right now we
     * just generate a complete random sequence using the given input symbols. Please change it to
     * your own code.
     */
  ...
  static void run() {
    ...
    while (System.currentTimeMillis() < END_TIME) {
      ...
      NUM_RUNS++;
      data.add(
          new String[] {Integer.toString(NUM_RUNS), Integer.toString(totalVisitedBranches.size()),
              Integer.toString(totalErrorsEncountered.size())});
      PathTracker.reset();
    }
  ...
  static String convertToCSV(String[] data) {
    return Stream.of(data).map(SymbolicExecutionLab::escapeSpecialCharacters)
  ...
  static void writeToCSV() throws IOException {
    ...
    try (PrintWriter pw = new PrintWriter(csvOutputFile)) {
      data.stream().map(SymbolicExecutionLab::convertToCSV).forEach(pw::println);
    }
  }
  ...
  static void writeToFile() {
    try {
      FileWriter myWriter =
          new FileWriter("docs/lab1/" + PROBLEM + "/problem_errors_Symbolic_Execution.txt", true);
